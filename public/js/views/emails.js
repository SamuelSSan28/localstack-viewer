...
export async function renderEmails(container) {
  ...
  try {
    ...
    const open = (index) => {
      ...
      document.querySelector('#mail-reader').innerHTML = email ? `<div class="reader-head"><span class="eyebrow">MESSAGE</span><h2>${escapeHtml(email.subject)}</h2><p><b>From:</b> ${escapeHtml(email.from)}</p><p><b>To:</b> ${escapeHtml([].concat(email.to).join(', '))}</p></div><pre>${escapeHtml(email.body || JSON.stringify(email.raw, null, 2))}</pre>` : '<div class="empty"><b>Select a message</b></div>';
    };
