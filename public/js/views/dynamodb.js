...
function renderTableList(container, tables) {
  ...
  list.innerHTML = visible.map((table) => `<div class="table-option-row ${table === currentTable ? 'active' : ''}"><button class="table-option" data-table="${escapeHtml(table)}"><span>▦</span><b>${escapeHtml(table)}</b></button><button class="pin-button ${state.pinnedTables.includes(table) ? 'pinned' : ''}" data-pin="${escapeHtml(table)}" title="${state.pinnedTables.includes(table) ? 'Unpin table' : 'Pin table'}" aria-label="${state.pinnedTables.includes(table) ? 'Unpin' : 'Pin'} ${escapeHtml(table)}">★</button></div>`).join('') || '<div class="list-empty">No matching tables</div>';
