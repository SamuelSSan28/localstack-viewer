...
test('lists S3 buckets and object metadata from XML responses', async (context) => {
  ...
  global.fetch = async (url) => {
    if (url.endsWith('/')) return new Response('<ListAllMyBucketsResult><Buckets><Bucket><Name>assets</Name><CreationDate>2026-08-12T00:00:00Z</CreationDate></Bucket></Buckets></ListAllMyBucketsResult>');
    if (url.endsWith('/assets?location')) return new Response('<LocationConstraint xmlns="http://s3.amazonaws.com/doc/2006-03-01/">sa-east-1</LocationConstraint>');
    return new Response('<ListBucketResult><Contents><Key>images/logo final.png</Key><LastModified>2026-08-12T12:00:00Z</LastModified><ETag>&quot;abc123&quot;</ETag><Size>2048</Size><StorageClass>STANDARD</StorageClass></Contents></ListBucketResult>');
  };

  assert.deepEqual(await listBuckets(), [{ name: 'assets', createdAt: '2026-08-12T00:00:00Z', region: 'sa-east-1' }]);
...
test('normalizes default and legacy S3 bucket regions without losing inaccessible buckets', async (context) => {
  ...
  context.after(() => { global.fetch = originalFetch; });
  global.fetch = async (url) => {
    if (url.endsWith('/')) return new Response('<ListAllMyBucketsResult><Buckets><Bucket><Name>default</Name></Bucket><Bucket><Name>legacy</Name></Bucket><Bucket><Name>private</Name></Bucket></Buckets></ListAllMyBucketsResult>');
    if (url.endsWith('/default?location')) return new Response('<LocationConstraint/>');
    if (url.endsWith('/legacy?location')) return new Response('<LocationConstraint>EU</LocationConstraint>');
