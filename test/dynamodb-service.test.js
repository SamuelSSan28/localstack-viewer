...
test('reads every DynamoDB scan page and returns lossless type schemas', async (context) => {
  ...
  global.fetch = async (_url, options) => {
    ...
    if (!body.ExclusiveStartKey) {
      return Response.json({ Items: [{ id: { N: '9007199254740993' } }], LastEvaluatedKey: { id: { N: '9007199254740993' } } });
    }
    return Response.json({ Items: [{ id: { N: '9007199254740994' }, profile: { M: { score: { N: '1.0000000000000001' } } } }] });
  };

  const result = await getTable('users');
  assert.equal(scanRequests, 2);
  assert.equal(result.count, 2);
  assert.deepEqual(result.items.map((item) => item.id), ['9007199254740993', '9007199254740994']);
