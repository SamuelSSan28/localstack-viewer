...
export async function listTables() {
  return ((await dynamoRequest('ListTables')).TableNames || []).filter(
    (tableName) => tableName !== messageStoreTable,
  );
}
  ...
  if (type === 'M')
    return {
      ...
      fields: Object.fromEntries(
        Object.entries(value.M).map(([name, child]) => [name, attributeSchema(child)]),
      ),
    };
...
async function scanAll(tableName) {
  ...
  let startKey;
  do {
    const page = await dynamoRequest('Scan', {
      TableName: tableName,
